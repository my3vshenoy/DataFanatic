...
random.seed(10)
print([random.randint(0,10) for _ in range(3)])
...
average = numpy.mean(large_sample)

deviations_from_mean = (.18 - average)/large_sample_std
...
greater_than_675 = len([m for m in mean_ratios if m >= .675])

significance_value = greater_than_675 / len(mean_ratios)
