...
median_difference = [i - values_median for i in values]
...
#Calculating the variance of the data set's "pts" column, which holds the total number of points each player scored.
pts_mean = nba_stats["pts"]/len(nba_stats["pts"])
variance_pts = 0
...
def calc_column_deviation(data):
    mean  = data.mean()
    variance = 0
    for p in data:
        diff = p - mean
...
#Calculate the proportion of the data that's within one, two and three standard deviation of the mean.
def what_percentage(std_dev_distances, count):
    x = []
    for i in std_dev_distances:
        if i <= count and i >= -count:
            x.append(i)
            
    return len(x)/len(std_dev_distances)
    
within_one_percentage = what_percentage(std_dev_distances, 1)
within_two_percentage = what_percentage(std_dev_distances, 2)
within_three_percentage = what_percentage(std_dev_distances, 3)
...
def calc_covariance(x, y):
    ...
    covariance = [x_diffs[i] * y_diffs[i] for i in range(len(x))]
    return sum(covariance) / len(covariance) #r
    
cov_stl_pf = calc_covariance(nba_stats["stl"], nba_stats["pf"])

cov_fta_pts = calc_covariance(nba_stats["fta"], nba_stats["pts"])
...
std_dev_fta = nba_stats["fta"].std()
std_dev_blk = nba_stats["blk"].std()
r_fta_blk = cov_fta_blk / (std_dev_fta * std_dev_blk)
...
std_dev_ast = nba_stats["ast"].std()
std_dev_stl = nba_stats["stl"].std()
r_ast_stl = cov_ast_stl / (std_dev_ast * std_dev_stl)
